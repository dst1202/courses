...
```markdown
(.*)  ## confusingly matches an pair of brackets with something or nothing in between without escaping the need to escape the brackets
```
...
```markdown
[0-9]+ (.*)[0-9]+  ## at least one number, space, anything, at least one number
```
...
```markdown
[Bb]ush( +[^ ]+ +){1,5} debate ## Bush followed by space, not space, space between 1 & 5 times, followed by debate
```
...
```markdown
+([a-zA-Z]+) +\1 + ## matches at least one word followed by at leaat one space, followed by the same word \1
```
